...
const form = useForm({
    ...
    tags:        props.notice.tags ?? [],
    image:       null,
    is_pinned:   props.notice.is_pinned ?? false,
});
...
const handlePopState = () => {
    if (form.isDirty) {
        if (!confirm('공지사항을 수정 중입니다. 정말 나가시겠습니까?')) {
...
const removeRouterListener = router.on('before', (event) => {
    if (form.isDirty && !confirm('공지사항을 수정 중입니다. 정말 나가시겠습니까?')) {
        event.preventDefault();
    }
});
...
const imagePreview = ref(
    props.notice.card_image_path
        ? `/storage/${props.notice.card_image_path}`
        : null
);
...
<template>
    <Head title="공지사항 수정 - Admin" />

    <AdminLayout>
        <div class="mb-8 flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 border-b-4 border-amber-500 inline-block pb-1">공지사항 수정</h1>
                <p class="mt-2 text-sm text-gray-600">공지사항 내용을 수정합니다.</p>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- 카테고리 -->
                    <div>
                        <label for="category_id" class="block text-sm font-bold text-gray-700 mb-2">공지 카테고리 <span class="text-amber-500 text-xs font-bold ml-1">· 공지 전용만 표시</span></label>
                        ...
                        <div v-else class="flex items-center gap-2 p-3 bg-red-50 border border-red-200 rounded-xl text-sm text-red-700 font-bold">
                            <span>⚠️</span>
                            <span>공지 전용 카테고리가 없습니다. 카테고리를 먼저 추가해주세요.</span>
                        </div>
                ...
                <!-- 제목 -->
                <div>
                    <label for="title" class="block text-sm font-bold text-gray-700 mb-2">공지 제목</label>
                    <input
                        id="title"
                        v-model="form.title"
                        type="text"
                ...
                <!-- 태그 -->
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">태그 (최대 3개)</label>
                    <div class="flex flex-wrap gap-2 mb-2">
                        <span
                            v-for="(t, index) in form.tags"
                            :key="index"
                            class="bg-amber-50 text-amber-700 px-3 py-1 rounded-full text-sm font-bold flex items-center border border-amber-100 shadow-sm"
                        >
                            #{{ t }}
                ...
                <!-- 본문 -->
                <div>
                    <label for="content" class="block text-sm font-bold text-gray-700 mb-2">공지 본문</label>
                    <TiptapEditor id="content" v-model="form.content" required />
                    <div v-if="form.errors.content" class="text-red-500 text-sm mt-1 font-bold">{{ form.errors.content }}</div>
                </div>

                <!-- 썸네일 이미지 -->
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">공지 썸네일</label>

                    <!-- 기존 이미지 미리보기 -->
                    <div v-if="imagePreview" class="mb-3 relative inline-block">
                        <img :src="imagePreview" class="h-32 w-auto rounded-xl object-cover border border-gray-200 shadow-sm" alt="썸네일 미리보기" />
                        <button
                            type="button"
                            @click="removeImage"
                    ...
                    <p v-if="!imagePreview && notice.card_image_path" class="text-xs text-gray-400 mt-1">새 이미지를 선택하면 기존 이미지가 교체됩니다.</p>
                ...
                <div class="flex justify-end pt-6 border-t border-gray-100 gap-4">
                    ...
                    <button
                        ...
                        class="px-10 py-2.5 bg-amber-500 text-white rounded-xl font-black shadow-lg shadow-amber-200 hover:bg-amber-600 transition-all disabled:opacity-50 flex items-center gap-2"
                    >
