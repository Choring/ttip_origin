  ...
  <section v-if="spots && spots.length > 0">
    ...
    <div class="relative -mx-4 px-4">
      <div class="flex gap-3 overflow-x-auto pb-3 no-scrollbar scroll-smooth snap-x snap-mandatory">
        ...
        >
          <!-- 이미지 (와이드 비율) -->
          <div class="relative w-full h-28 rounded-xl overflow-hidden mb-2 bg-gray-100">
            <img
              :src="spot.image"
              :alt="spot.title"
          ...
          <!-- 주소 -->
          <p class="text-[11px] text-gray-400 line-clamp-1">📍 {{ shortAddr(spot.addr1) }}</p>
        </Link>

        <!-- 더보기 카드 -->
        <Link
          :href="route('tour.index')"
          class="flex-shrink-0 w-44 snap-start h-28 rounded-xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center gap-2 hover:border-orange-300 hover:bg-orange-50 transition-all group"
        >
