<template>
  <section v-if="events && events.length > 0">
    ...
    <div class="relative -mx-4 px-4">
      <div class="flex gap-3 overflow-x-auto pb-3 no-scrollbar scroll-smooth snap-x snap-mandatory">
        <Link
          v-for="event in events"
          :key="event.event_seq"
          :href="route('events.show', event.event_seq)"
          class="flex-shrink-0 w-44 snap-start group"
        >
          <!-- 이미지 -->
          <div class="relative w-full h-56 rounded-xl overflow-hidden mb-2 bg-gray-100">
            <img
              :src="event.image"
              :alt="event.subject"
          ...
          <!-- 텍스트 -->
          <p class="text-xs font-black text-gray-900 line-clamp-2 leading-snug mb-1 group-hover:text-orange-500 transition-colors">
            {{ event.subject }}
          </p>
          <p class="text-[11px] text-gray-400 line-clamp-1">📍 {{ event.place }}</p>
          <p class="text-[11px] text-gray-400 mt-0.5">{{ formatDate(event.start_date) }}</p>
        </Link>

        <!-- 더보기 카드 -->
        <Link
          :href="route('events.index')"
          class="flex-shrink-0 w-44 snap-start h-56 rounded-xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center gap-2 hover:border-orange-300 hover:bg-orange-50 transition-all group"
        >
